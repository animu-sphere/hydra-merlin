...
| MaterialXGenSlang material-function prototype | In progress for v0.10.0: optional `Merlin::MaterialX` emits deterministic graph-only Slang functions and renderer-owned minimum Standard Surface results for constants, image/UV0/world-normal, add/multiply/mix, `base`, `base_color`, `metalness`, `specular_roughness`, and `normal`. Portable standard-library/include fingerprints feed topology-only SHA-256 module keys, which are separate from parameter/resource-state keys, typed host-neutral Core metadata, exact input requirements, logical reflection, direct SPIR-V/Metal compile gates, and the shared target-artifact key. Registered parameter-only artifacts execute in renderer-owned Vulkan Forward after ABI/reflection checks, reuse pipelines across value edits, and report structured runtime fallback/capability telemetry. Generated texture/sampler binding, the complete image matrix, host/report serialization, and packaged artifact evidence remain incomplete |

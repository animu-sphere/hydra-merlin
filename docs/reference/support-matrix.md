...
**Status:** v0.15.0 released · **Last reviewed:** 2026-08-11
...
| GPU-driven indexed Mesh submission | In progress for v0.16.0: the backend-neutral CPU oracle validates persistent candidates, visibility/frustum culling, stable compaction identity, arena-block-safe indirect command batches, and exact counters. An opt-in Vulkan runtime now executes the deterministic compute kernel and device-local indirect/count output for one arena/pipeline batch, with visible/culled runtime evidence. Multi-batch grouping, persistent candidate reuse, parallel compaction, settings integration, and draw-count-independent CPU-cost evidence remain incomplete |

...
| HgiVulkan host presentation bridge | In progress for v0.13.0: public-driver discovery and the Hgi-owned Tier 0 upload target path compile on OpenUSD 26.05/26.08 with structured fallback and target/upload telemetry. The validated local packages omit a Vulkan Hgi driver, so runtime target-upload evidence remains pending and those packages retain the CPU RenderBuffer path. Selected-AOV GPU copy and distinct bridge/host-consumption completion remain unavailable. Direct same-device sharing is a separately evidence-gated v0.13.1 capability. |

...
| MaterialXGenSlang material-function prototype | In progress for v0.10.0: optional `Merlin::MaterialX` emits deterministic graph-only Slang functions, topology-only SHA-256 module keys, separate parameter/resource-state keys, typed host-neutral Core state/layout metadata, exact object/world/UV0 input requirements, logical reflection, and local diagnostics for constants and add/multiply/mix; direct wrappers compile the same source for SPIR-V and Metal. Image/texcoord/normal, minimum Standard Surface, standard-library/include fingerprints, target-artifact identity, `merlin-diagnostic/v1` fallback, Vulkan Forward execution/image parity, and final artifact evidence remain incomplete |

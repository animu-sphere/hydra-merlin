...
**Status:** v0.15.0 released · **Last reviewed:** 2026-08-22
...
| GPU-driven indexed Mesh submission | In progress for v0.16.0: the backend-neutral CPU oracle validates persistent candidates, visibility/frustum culling, stable compaction identity, arena-block-safe indirect command batches, and exact counters. Renderer settings v2 exposes disabled, preferred-with-fallback, and required selection plus culling controls through the backend-neutral request. The opt-in Vulkan runtime retains candidate lists per reusable frame context, partitions consecutive arena/pipeline batches into frame-shared buffers and descriptors, classifies candidates in 64-thread workgroups, atomically compacts device-local indirect commands, and executes one indirect-count draw per batch. Metal rejects required selection and reports preferred fallback. Validation covers mixed pipelines, 32 alternating batches without proportional native allocations, and 130 candidates across multiple workgroups. Draw-count-independent large-scene CPU-cost evidence remains incomplete |
...
| Standard OpenUSD Gaussian ingestion and rendering | Available: the v0.14.1 Vulkan correctness MVP carries OpenUSD 26.05/26.08 `ParticleField3DGaussianSplat` through Hydra `particleField`, deterministic CPU preparation, procedural ellipse rasterization, mixed Mesh composition, IDs/picking, and reference evidence. v0.15.0 adds generation-checked persistent device-local position/covariance/opacity/SH ranges, zero-work static reuse, range-only edits, separated timing, and 1M/5M/10M fixtures. GPU-driven projection/sorting remains v0.16.0. |
...
| GPU-driven indexed | `drawIndirectFirstInstance`, `drawIndirectCount`, and `shaderDrawParameters` are enabled by an owned device or declared enabled by a borrowed host; the request fits the current arena/pipeline batch boundary; indirect identity, compute culling, image parity, and performance gates pass | Table-backed or conventional indexed submission |

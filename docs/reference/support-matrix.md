...
| GPU-driven indexed Mesh submission | In progress for v0.16.0: the backend-neutral CPU oracle now validates persistent candidates, visibility/frustum culling, stable compaction identity, indirect commands, and exact counters. Vulkan compute and multi-draw indirect runtime execution are not yet available; current Mesh submission is not claimed to have draw-count-independent CPU cost |

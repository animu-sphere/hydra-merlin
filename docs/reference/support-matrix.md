...
| GPU-driven indexed Mesh submission | In progress for v0.16.0: the backend-neutral CPU oracle validates persistent candidates, visibility/frustum culling, stable compaction identity, arena-block-safe indirect command batches, and exact counters. The opt-in Vulkan runtime retains candidate lists per reusable frame context, partitions consecutive arena/pipeline batches into frame-shared buffers and descriptors, classifies candidates in 64-thread workgroups, atomically compacts device-local indirect commands, and executes one indirect-count draw per batch. Validation covers mixed pipelines, 32 alternating batches without proportional native allocations, and 130 candidates across multiple workgroups. Public settings integration and draw-count-independent large-scene CPU-cost evidence remain incomplete |

...
function(_merlin_depfile_module_paths _depfile _output)
  ...
  foreach(_token IN LISTS _tokens)
    ...
    if(NOT EXISTS "${MERLIN_SHADER_SOURCE_DIR}/${_name}" AND
       (NOT DEFINED MERLIN_SHARED_SHADER_SOURCE_DIR OR
        NOT EXISTS "${MERLIN_SHARED_SHADER_SOURCE_DIR}/${_name}"))
      message(FATAL_ERROR
        "Shader depfile ${_depfile} names a source outside "
        "the renderer/shared shader roots: ${_token}")
    endif()
...
function(_merlin_module_identity _paths _identity_output _sources_json_output)
  ...
  foreach(_path IN LISTS _paths)
    set(_physical_path "${MERLIN_SHADER_SOURCE_DIR}/${_path}")
    if(NOT EXISTS "${_physical_path}" AND
       DEFINED MERLIN_SHARED_SHADER_SOURCE_DIR)
      set(_physical_path "${MERLIN_SHARED_SHADER_SOURCE_DIR}/${_path}")
    endif()
    if(NOT EXISTS "${_physical_path}")
      message(FATAL_ERROR "Shader module source is missing: ${_path}")
    endif()
    file(SHA256 "${_physical_path}" _hash)

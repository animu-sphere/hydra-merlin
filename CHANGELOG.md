...
- The first Vulkan GPU-driven Gaussian preparation contract reads the existing
  ...
  v6 fixes the C++/Slang uniform-backed constants, prepared-record, counter,
  and descriptor layouts through SPIR-V reflection and packages the compute
  artifact while the CPU-sorted raster path remains the runtime fallback.
...
- A Vulkan GPU-driven indexed compute and opt-in renderer
  ...
  counter layout. The first single-arena/pipeline runtime slice writes
  device-local indirect/count buffers, executes them through
  `vkCmdDrawIndexedIndirectCount`, and resolves the stable physical draw slot
  from `firstInstance` in dedicated Forward stages. Vulkan device creation and
  borrowed-host negotiation now explicitly enable and report
...
- Versioned persistent GPU Scene residency for geometry, instance, and
  ...
  plans expose the same dirty-range contract, and resource-record changes now
  reissue only their dependent draws so fresh physical resource slots cannot
  leave current draw records pointing at retired generations.
...
- Native platform USD file selection in the Hydra development viewport,
  ...
  generate a launcher with the configured OpenUSD runtime path.
